...
int udt_startup() {
    send_buffer_init();   # send_buffer.c 中   都是调用 buffer.c 中 buffer_init
    recv_buffer_init();		# recv_buffer.c 中
    return 0;
}


# int socket(int domain, int type, int protocol);    protocal为0的时候自动选择
...
# thread_start 在util中 多线程调用   receiver_start 和 sender_start 分别在receiver和sender中
# receiver_start 循环接受 信息 并解析信息 packet_parse 并写入到buffer中去
    thread_start((thread_worker_t) receiver_start, (&connection));
                     ...
                     int blocksize)
{
	...
	# send file buffer write在 send buffer中
